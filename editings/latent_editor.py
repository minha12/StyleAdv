...
class LatentEditor(object):
    ...
    def apply_ganspace(self, latent, ganspace_pca, edit_directions):
        ...
        # print("inside apply_ganspace ", edit_latents.shape)
        return self._latents_to_image(edit_latents), edit_latents

    def apply_multi_ganspace(self, latent, ganspace_pca, edit_directions):
        edit_latents = ganspace.edit_at_once(latent, ganspace_pca, edit_directions)
        # print("inside apply_ganspace ", edit_latents.shape)
        return self._latents_to_image(edit_latents), edit_latents
    ...
    def _latents_to_image(self, latents):
        with torch.no_grad():
            images, _ = self.generator(
                [latents], None, randomize_noise=False, input_is_latent=True
            )
            if self.is_cars:
