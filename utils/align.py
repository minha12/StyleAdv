...
def process_input_image(image_path, run_align=True):
    ...
    if run_align:
        ...
        try:
            ...
            aligned_image = align_face(
                filepath=image_path, predictor=predictor, cnn_detector=True
            )
...
def get_landmark(filepath, predictor):
    ...
    closest_face = min(
        ...
        key=lambda det: np.linalg.norm(
            center_img
            - np.array([det.left() + det.width() / 2, det.top() + det.height() / 2])
        ),
    )

    shape = predictor(img, closest_face)
...
def get_landmark_cnn(inp, predictor, input_is_path=True):
    """get landmark with dlib
    :return: np.array shape=(68, 2) of the largest face
    """
    detector = dlib.cnn_face_detection_model_v1(
        "./pretrained_models/mmod_human_face_detector.dat"
    )
    if input_is_path:
        inp = dlib.load_rgb_image(inp)
    dets = detector(inp, 1)
    ...
    closest_face = min(
        ...
        key=lambda det: np.linalg.norm(
            ...
            - np.array(
                [
                    det.rect.left() + det.rect.width() / 2,
                    det.rect.top() + det.rect.height() / 2,
                ]
            )
        ),
    )

    # Find landmarks for the center face
    shape = predictor(inp, closest_face.rect)
...
def align_face(filepath, predictor, cnn_detector=False):
    """
    :param filepath: str
    :return: PIL Image
    """
    if cnn_detector:
        lm = get_landmark_cnn(filepath, predictor)
    else:
